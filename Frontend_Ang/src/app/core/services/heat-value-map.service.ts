...
export class HeatValueMapService implements OnDestroy, MapLayer {
  ...
  clear(map?: maplibregl.Map) {
    ...
    if (m.getSource(this.sourceId)) {
      try { m.removeSource(this.sourceId); } catch { }
    }
  ...
  private resolveOptions(opts: HeatValueOptions): ResolvedHeatValueOptions {
    ...
    if (opts.radiusStops && opts.radiusStops.length) {
      ...
      radiusStops = [[10, opts.radiusRange.min], [15, opts.radiusRange.max]];
    } else {
      radiusStops = [[10, 18], [13, 30], [15, 50]];
    }

    const intensityStops: Array<[number, number]> = opts.intensityStops?.length
      ? opts.intensityStops
      : [[10, 0.8], [15, 1.6]];

    const colors = opts.colorRamp?.length ? opts.colorRamp.slice() : DEFAULT_HEAT_COLORS.slice();
  ...
  private buildRadiusExpression(stops: Array<[number, number]>): any[] {
    ...
    for (const [z, r] of stops) expr.push(z, r);
    return expr;
  }
  ...
  private buildIntensityExpression(stops: Array<[number, number]>): any {
    ...
    const expr: any[] = ['interpolate', ['linear'], ['zoom']];
    for (const [z, value] of stops) expr.push(z, value);
    return expr;
  }
}
